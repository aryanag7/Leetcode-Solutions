...
class Solution:
    ...
    # zeros<=k but when new ele is 0 3<=2, we just need to remove one element from the left (potentially zero) to make it a valid window again and making sure that the WINDOW SIZE  dont go DOWN BEYOND our ans length (IMPPPPPP). we will increase it if condition satisified but will not let it go beyond what we found ans length so far.
